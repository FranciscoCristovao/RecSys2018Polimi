...
full_data_sequential = pd.read_csv('../input/tsequential/train_sequential.csv', sep=',')
...
while num_factors < 1000:
    rs.fit(num_factors=num_factors)
    predictions = rs.recommend(target_data['playlist_id'])
    print(num_factors)
    map_temp = evaluator.evaluate(predictions, test_data)
    factors_list.append({'map': map_temp, 'num_factors':num_factors})
    num_factors += 10

print(factors_list, 'knn = 100')
# l1=[{'map':1, 'knn':50},{'map':2, 'l1_ratio':50}]
df = pd.DataFrame(l1_list, columns=['map', 'num_factors'])
df.to_csv('num_factors_pureSVD.csv', ',')
