...
class HybridRS:
    ...
    def __init__(self, tracks_data, at=10, k_con=10, k_col_u_u=200, k_col_i_i=700,
    ...
    def recommend_linear(self, playlist_ids, alpha=1, beta=5, gamma=7, delta=0.9, filter_top_pop=False):
        print("Recommending... Am I filtering top_top songs?", filter_top_pop)
        ...
        e_r_col_i_i = self.col_i_i_recommender.get_estimated_ratings()
        e_r_col_u_u = self.col_u_u_recommender.get_estimated_ratings()
        e_r_pureSVD = self.pureSVD_recommender.get_estimated_ratings()
        print(type(e_r_pureSVD))
        ...
        for k in playlist_ids:
            try:
                ...
                mf_row = np.multiply(self.pureSVD_recommender.compute_score_SVD(k), delta)
    ...
    def recommend_francisco(self, playlist_ids, alpha=1, beta=5, gamma=7, delta=0.9):
        print("Recommending...")

        final_prediction = {}
        counter = 0
        print("Entering pureSVD")
        e_r_pureSVD = self.pureSVD_recommender.get_estimated_ratings()
        print(e_r_pureSVD)
        print(type(e_r_pureSVD))
        print("pureSVD done")
        ...
        e_r_base_hybrid = e_r_col_u_u.multiply(alpha)\
                                + e_r_col_i_i.multiply(beta)\
                                + e_r_cbf.multiply(gamma)

        estimated_ratings_final = e_r_base_hybrid+e_r_pureSVD.multiply(delta)

        print("FINALLY RECOMMENDING")
        '''
        print("CBF")
        print(e_r_cbf[7].data[e_r_cbf[7].data.argsort()[::-1]])
        print("COL_I_I")
        ...
        print(e_r_col_u_u[7].data[e_r_col_u_u[7].data.argsort()[::-1]])
        print("Hybrid")
        print(e_r_base_hybrid[7].data[e_r_base_hybrid[7].data.argsort()[::-1]])
        print("pureSVD")
        print(e_r_pureSVD[7].sort_values(ascending=False))
        '''
